# -*- coding: utf-8 -*-
"""Scenario_1完整流程: FP-USD 500K 完整19步骤 (REG环境)"""
...
DB_CONFIG = {"host": "18.162.145.173", "user": "dpu_sit", "password": "20250818dpu_sit", "database": "dpu_seller_center", "port": 3306, "charset": "utf8mb4"}
faker = Faker("zh_CN")
...
def run_scenario_1():
    ...
    # 步骤4-8: SP和3PL授权
    log.info("SP和3PL授权", extra={"step": "4-8"})
    ...
    result = wait_db(f"SELECT idempotency_key, platform_offer_id FROM dpu_manual_offer WHERE platform_seller_id='{vars['selling_partner_id']}' LIMIT 1", "platform_offer_id", timeout=120)
    ...
    log.info(f"SP/3PL授权完成", extra={"step": "4-8"})

    # 步骤9-14: 创建申请单和触发后台任务
    log.info("创建申请单", extra={"step": "9-14"})
    ...
    result = wait_db(f"SELECT authorization_id FROM dpu_auth_token WHERE merchant_id='{vars['merchantId']}' AND authorization_party='SP' AND status='ACTIVE' LIMIT 1", "account_id", timeout=60)
    ...
    result = wait_db(f"SELECT limit_application_unique_id FROM dpu_limit_application WHERE merchant_id='{vars['merchantId']}' LIMIT 1", "limit_application_id", timeout=90)
    vars["dpuLimitApplicationId"] = result['limit_application_unique_id']
    log.info(f"申请单创建完成", extra={"step": "9-14"})
    ...
    report = {
        "environment": "reg",
        "scenario": "scenario_1_full_19_steps (FP-USD-500k)",
    ...
    with open('scenario_1_reg_full_19_report.json', 'w', encoding='utf-8') as f:
        json.dump(report, f, indent=2, ensure_ascii=False)

    log.info("="*60, extra={"step": "完成"})
    log.info(f"✅ 完成! phone: {vars['phone']}, merchant_id: {vars['merchantId']}", extra={"step": "完成"})
    log.info(f"报告: scenario_1_reg_full_19_report.json", extra={"step": "完成"})
