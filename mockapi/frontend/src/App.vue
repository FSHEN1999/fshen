...
const filteredInterfaceScenarios = computed(() => {
  ...
  if (!keyword) return interfaceScenarios.value
  return interfaceScenarios.value
    .map((scenario) => {
...
function getScenarioStepTimeout(step) {
  if (step.endpoint === '/api/mock/fp-scheduled-submit') return 30000
  return undefined
}
...
async function runScheduledSubmitScenarioStep(step, payload, startedAt) {
  ...
  return {
    success: false,
    error: 'scheduled-submit 后台任务轮询超时，未拿到最终状态',
    job_id: jobId,
    ...
    <section v-if="!isAuthenticated" class="login-view">
      <el-card shadow="never" class="login-panel">
        <div class="login-scene" :class="{ active: authMode === 'register' }">
          <img class="login-bg" src="./assets/login-hero.png" alt="" aria-hidden="true" />
          ...
          <div class="login-area">
            <div class="login-access">
              ...
              <p class="login-subtitle">使用账号密码进入 Mock 控台</p>
              <el-form label-position="top" class="tight-form login-form" @submit.prevent="handleLogin">
                <div class="login-inputs">
                  <el-form-item label="账号">
                    <el-input v-model.trim="loginForm.username" :prefix-icon="User" autocomplete="username" placeholder=" " />
                  </el-form-item>
                ...
                <el-button class="login-main-button" type="primary" native-type="submit">登 录</el-button>
                <p class="login-switch">还没有账号？<button type="button" @click="openRegisterPage">创建账号</button></p>
              </el-form>
            </div>

            <div class="login-register">
              <h1 class="login-title">创建账号</h1>
              <p class="login-subtitle">注册一个新账号以使用 Mock 控台</p>
              <el-form label-position="top" class="tight-form login-form register-form" @submit.prevent="handleUserRegister">
                <div class="login-inputs">
                  <el-form-item label="账号">
                    <el-input v-model.trim="userRegisterForm.username" :prefix-icon="User" autocomplete="username" placeholder=" " />
                  </el-form-item>
                  <el-form-item label="密码">
                    <el-input
                ...
                <el-button class="login-main-button" type="primary" native-type="submit">注 册</el-button>
                <p class="login-switch">已有账号？<button type="button" @click="openLoginPage">登录</button></p>
              </el-form>
            </div>
          </div>
        </div>
    ...
    <section v-if="currentView === 'interfaceTest'" class="interface-test-view">
      ...
      <div class="interface-automation-shell">
        <aside class="scenario-library" aria-label="场景集合">
          <div class="scenario-tree">
            <div class="scenario-tree-head">
              <strong>全部场景 ({{ filteredInterfaceScenarios.length }})</strong>
              <el-tag size="small" effect="plain">DPU产品流程</el-tag>
            </div>

            <div
              v-for="scenario in filteredInterfaceScenarios"
              :key="scenario.key"
            ...
            >
              ...
              <ul v-if="expandedScenarioTrees[scenario.key]" class="scenario-tree-step-list">
                <li
                  ...
                  :class="{
                    active: activeInterfaceScenarioKey === scenario.key && activeOperationKey === step.key && interfaceFocusMode === 'step',
        ...
        <main class="scenario-workbench">
          ...
          <div v-if="interfaceFocusMode === 'scenario'" class="scenario-tabs">
            <button
          ...
          <template v-if="interfaceFocusMode === 'scenario'">
          ...
          <template v-else-if="interfaceFocusMode === 'step' && selectedInterfaceStep">
            <section class="step-editor">
              <div class="step-editor-head">
                ...
                <button
                  ...
                  type="button"
                  :disabled="(selectedInterfaceStep.type === 'api' && !activeSessionId) || runningOperationKey === selectedInterfaceStep.operation?.key"
                  @click="handleScenarioStepRun(selectedInterfaceStep)"
                >
              ...
              <section v-if="stepFocusTab === 'params'" class="step-editor-panel">
                <div class="scenario-description-grid">
                  ...
                  <div>
                    <span>Fields</span>
                    <strong>{{ getScenarioStepDescription(selectedInterfaceStep).fieldCount }} 个参数</strong>
                  </div>
                </div>
                <div v-if="getScenarioStepDescription(selectedInterfaceStep).fields.length" class="scenario-field-summary">
                  <div
              ...
              <section v-else-if="stepFocusTab === 'body'" class="step-editor-panel response-block">
                ...
                <pre>{{ JSON.stringify(getScenarioStepRequestBody(selectedInterfaceStep), null, 2) }}</pre>
              </section>
...
X-Mockapi-Session: {{ activeSessionId || '-' }}
X-Mockapi-Username: {{ authDisplayName || '-' }}</pre>
              </section>

              <section v-else class="step-editor-panel response-block">
                <div class="response-block-head">
