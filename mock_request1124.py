...
ENV = "uat"  # "sit" or "local" or "dev" or "uat" or "preprod"
...
class SanctionStatus(Enum):
    """application表，制裁状态：'HIT','NOT_HIT','INITIAL'"""
    HIT = 'HIT'
    NOT_HIT = 'NOT_HIT'
    INITIAL = 'INITIAL'
...
class MockDpuStatus:
    ...
    def mock_spapi_auth(self, ):
        # seller_id名称 第一家店1/第二家店2... + BTC6RWJD + phone_number
        while True:
            number = input(
    ...
    def get_merchant_id(self):
        ...
        sql = f"select merchant_id from dpu_users where phone_number = '{self.phone_number}' ORDER BY created_at DESC;"
    ...
    def get_platform_offer_id(self, seller_id):
        ...
        sql = f"SELECT platform_offer_id FROM dpu_manual_offer WHERE platform_seller_id = '{seller_id}'ORDER BY created_at DESC;;"
    ...
    def get_dpu_auth_token_seller_id(self):
        ...
        sql = f"select authorization_id from dpu_auth_token where merchant_id = '{self.merchant_id}' and authorization_party='SP' order by created_at desc"
    ...
    def mock_sanction_status(self, ):
        ...
        sanction_status = ''
        while True:
            input_ = input("请输入制裁状态对应的数字：1-NOT_HIT 2-HIT 3-INITIAL \n").strip()
            match input_:
                case '1':
                ...
                case '2':
                    ...
                    break
                case '3':
                    sanction_status = SanctionStatus.INITIAL.value
                    break
                case _:
                    log.error("输入错误，请重新输入: ")
                    continue
    ...
    def mock_approved_offer_status(self, journey: str = None, status: str = None):
        ...
        def approvedoffer_completed_request(param_approved_offer_status, param_approved_limit_amount,
                                            param_signed_limit_amount, param_failure_reason):
            # if not ENV == 'preprod':
            request_body = {
                "data": {
            ...
            if response.status_code == 200 and response.json().get("data") == {}:
                ...
                log.error(f"审批状态修改失败 {response.json()}")

        approved_offer_status = ''
        failure_reason = None
        while True:
            input_ = input("请输入审批状态对应数字：1-APPROVED 2.RETURNED 3-REJECTED \n").strip()
            match input_:
                case '1':
                    approved_offer_status = DPUStatus.APPROVED.value
                    break
                case '2':
                    approved_offer_status = DPUStatus.RETURNED.value
                    input_failure_reason = input(
                        "请输入拒绝原因：1-不正确brn 2-未能获取客户 ID 号码 3-ID 号码与 CR 记录不相符 4-未能通过公司结构校验 5-需要人工处理反洗钱验证\n").strip()
                    while True:
                        match input_failure_reason:
                            case "1":
                                failure_reason = ReturnedFailureReason.INCORRECT_BRN.value
                                break
                            case '2':
                                failure_reason = ReturnedFailureReason.FAILED_TO_OBTAIN_CUSTOMER_ID.value
                                break
                            case '3':
                                failure_reason = ReturnedFailureReason.ID_MISMATCH_WITH_CR_RECORD.value
                                break
                            case '4':
                                failure_reason = ReturnedFailureReason.FAILED_COMPANY_STRUCTURE_VERIFICATION.value
                                break
                            case '5':
                                failure_reason = ReturnedFailureReason.REQUIRES_MANUAL_AML_VERIFICATION.value
                                break
                            case _:
                                log.error("输入错误，请重新输入: ")
                                continue
    ...
    def mock_esign_status(self, ):
        ...
        def esign_completed_request(param_esign_status, param_signed_limit):
            ...
            if response.status_code == 200 and response.json().get("data") == {}:
                ...
                log.error(f"esign状态修改失败 {response.json()}")
    ...
    def mock_drawdown_status(self, status: str = None, step: int = 0):
        ...
        def disbursement_completed_request(param_drawdown_status, param_drawdown_amount):
            ...
            if response.status_code == 200 and response.json().get("data") == {}:
                ...
                log.error(f"提款状态修改失败 {response.json()}")
    ...
    def mock_underwritten_status(self, status: str = None, step: int = 0):
        ...
        def underwritten_request(param_underwritten_status, param_underwritten_amount, ):
            ...
            if response.status_code == 200 and response.json().get("data") == {}:
                ...
                log.error(f"额度评估失败 {response.json()}")
    ...
    def mock_psp_completed_status(self, ):
        ...
        def psp_verification_completed_request(param_psp_completed_status):
            ...
            if response.status_code == 200:
                ...
                log.error(f"psp验证完成状态修改失败... status_json: {response.json()}")
    ...
    def redirect_3pl(self):
        while True:
            ...
            if input_.isdigit() and int(input_) in range(1, 100):
                break
            else:
...
def is_registered(phone_number, executor):
    try:
        ...
        sql_ = f"select authorization_id from dpu_auth_token where merchant_id = '{merchant_id}' and authorization_party='3PL' order by created_at desc limit 1;"
        offer_id = executor.execute_sql(sql_)
        if offer_id:
            log.info(f"请核对offer_d:{offer_id}")
            return True
        log.error("请核对环境")
        if merchant_id:
            return True
...
def run():
    with ExecuteSql() as executor:
        while True:
            is_register_new_account = input("请输入是否注册新账号： 1-注册新账号 2-使用现有账号 \n").strip()
            match is_register_new_account:
                case "1":
        ...
        if is_register_new_account:
            ...
            while True:
                ...
                if phone_number.isdigit() and len(phone_number) in [11, 8]:
        ...
        while True:
            operator = input(
                "请输入回调的接口：1-spapi授权回调  2-link-sp-3pl 3-underwritten 4-approved  5-create_psp_record(不用做) "
...
if __name__ == '__main__':
    ...
    if ENV == "uat":
        ...
        REDIRECT_URL = f"https://preprod.api.expressfinance.business.hsbc.com/dpu-merchant/amazon/redirect"
    else:
